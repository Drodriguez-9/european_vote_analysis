...
import time
import pandas as pd
from src.data.make_dataset import split_data, load_data
...
def get_data(sigma_obs=0.05, N_test=500):
    ...
    X, Y, X_test, Y_test = X.values, Y.values, X_test.values, Y_test.values
    N, D_X = X.shape
    Y = Y[:, np.newaxis]
...
def main(args):
    ...
    mean_prediction_class = (mean_prediction >= 0.5).astype(int)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-samples", nargs="?", default=20, type=int)
    parser.add_argument("--num-chains", nargs="?", default=1, type=int)
    parser.add_argument("--num-hidden", nargs="?", default=2, type=int)
